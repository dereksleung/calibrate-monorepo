...
export async function writeLifecycle(entries: Array<[string, unknown]>) {
...
export async function readSnapshot(accountKey: string) {
  const database = await openDatabase();
  const transaction = database.transaction(DAY_LOG_CACHE_SNAPSHOT_STORE);
...
export async function readLifecycle(key: string) {
  const database = await openDatabase();
  const transaction = database.transaction(DAY_LOG_CACHE_LIFECYCLE_STORE);
