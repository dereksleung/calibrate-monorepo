import type { MealName } from "@calibrate/frontend-core/verticals/day-logs/models/day-log";
...
type FoodResultCardProps = {
  ...
  onQuickAdd?: (food: SelectedFoodForConfirmation, meal: MealName) => void;
    ...
    <li className="flex items-center gap-4 rounded-xl">
      ...
      {preselectedMeal ? (
        ...
        <Select.Root
          modal={false}
          onValueChange={(meal) => {
            if (meal) onQuickAdd?.(food, meal as MealName);
          }}
        >
          <Select.Trigger
