...
**What to build:** Dashboard and Logs compose cache-first Day Log views from account-scoped date slots and use bounded sync only when the user's active data needs validation. Logs reduces requests from passive historical browsing.
...
- [ ] Logs should read an existing slot from the cache when it is still fresh, and when not fresh, call the sync endpoint for a date range that is the date being browsed and 6 more days in the past, to sync the likeliest days the user may browse afterward to reduce further API requests.
