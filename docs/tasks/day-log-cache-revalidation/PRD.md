...
The cache must prevent another account's persisted wellness data from restoring or persisting after a successful logout, account transition, or confirmed session loss. [ADR-0004](../../adr/0004-bounded-day-log-sync-and-cache-fence.md) is the authoritative lifecycle and recovery protocol, including its durable fence, server-confirmed recovery phases, retry behavior, BroadcastChannel acceleration, and revocation guarantee.
...
- **Logs:** display a Sunday-to-Saturday Calendar week. The current Calendar week reuses Dashboard's rolling seven-day range; dates after today are Upcoming and disabled. Users cannot navigate to future Calendar weeks. When the snapped Calendar week is `W`, cache-first sync dates in `W` that are on or before today plus the previous Calendar week `W−1` ([ADR-0006](../../adr/0006-logs-calendar-week-scroller-prefetch.md)). When the user explicitly selects historical date `D`, compose the cache first and, only when slot `D` is `NotYetLoaded`, invalidated, or at least one hour past its `dataUpdatedAt`, sync `D - 6` through `D`. Check freshness of the selected date, not the whole week; accepted query data supplies the updated timestamps.
